...
#基于 Sinkhorn 迭代 的优化算法，用于计算两个表示张量之间的最优传输矩阵，
# 并最终返回一个相似性得分 S。它结合了 相似度矩阵 和 Sinkhorn 算法 来计算两个分布之间的匹配关系。以下是对代码的逐步讲解
...
class Model(nn.Module):
    ...
    def InterViewLoss(self, h1, h2, rescale_ratio, have_neg = False, neg_top_k = 50):
        ...
        if have_neg:
            ...
            neg_sim = neg_sim.to(torch.device('cuda:3' if torch.cuda.is_available() else 'cpu'))
    ...
    def forward(self, feature1, adj1, raw1, size1, feature2, adj2, raw2, size2,
        ...
        # return node_recons_1, node_recons_2, Inter_loss_1, Inter_loss_2, sim_all_1, sim_all_2, sim_pos_1, sim_pos_2
        return disc_1, disc_2, Inter_loss_1, Inter_loss_2, sim_all_1, sim_all_2, sim_pos_1, sim_pos_2
